...
export function Mnemonic() {
  ...
  const [accounts, setAccount] = useState<Array<{ publicKey: string; privateKey: string; address: string; path: string }>>([]);
  const [hdPath, setHdPath] = useState<string>("m/44'/309'/0'/0/0");

  const isValid = useMemo(() => hd.mnemonic.validateMnemonic(mnemonic), [mnemonic]);
  ...
  function expendPrivateKey(count: number, startIndex: number = 0) {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      const publicKey = hexFrom(secp256k1.getPublicKey(derivedKey.privateKey, true));
    ...
    (async () => {
      const newAccounts = await Promise.all(
        accounts.map(async (acc) => ({
          ...acc,
          address: await new ccc.SignerCkbPublicKey(client, acc.publicKey).getRecommendedAddress()
        }))
      );
      ...
      {accounts.length > 0 && (
        <div className="mt-2 w-full overflow-scroll whitespace-nowrap">
          ...
          {accounts.map(({ privateKey, address, path }) => (
            <p key={path}>{path}, {address}, {privateKey}</p>
          ))}
        </div>
      )}
    </div>
